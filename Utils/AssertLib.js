class AssertLib {
  ...
  static assertTypeArgs(argsData) {
    ...
    if (!(argsData instanceof Array)) {
      throw Error(
        'assertTypeArgs(): expected argsData as an Array but found something else.',
      );
    }
    ...
    if (!(boolFirstItemString || isArray(firstItem))) {
      throw Error(
        `assertTypeArgs(<rgsData>): expected argsData[0] to be a 'string' or Array but found something else.`,
      );
    }
    ...
    argsData.forEach((item, index) => {
      ...
      if (!isArray(item)) {
        throw Error(
          `assertTypeArgs(<rgsData.forEach>): expected argsData[${index + 1}] to be an Array but found something else.`,
        );
      }

      if (item.length != 3) {
        throw Error(
          `assertTypeArgs(<rgsData.forEach>): expected argsData[${index + 1}].length to be 3 but found ${item.length}.`,
        );
      }
      ...
      if (!isString(argName)) {
        throw Error(
          `assertTypeArgs(<rgsData.forEach>): expected argsData[${index + 1}][0] to be a 'string' but found something else.`,
        );
      }
      ...
      if (!(boolAcceptedTypesString || boolAcceptedTypesArray)) {
        throw Error(
          `assertTypeArgs(<rgsData.forEach>): expected argsData[${index + 1}][2] to be a 'string' or Array but found something else.`,
        );
      }
      ...
      if (boolAcceptedTypesString) {
        if (argValueType != acceptedTypes) {
          throw Error(
            `${srcName}: expected ${argName} to be ${acceptedTypes} but found ${argValueType}`,
          );
        }
      } else if (boolAcceptedTypesArray) {
        if (!acceptedTypes.includes(argValueType)) {
          throw Error(
            `${srcName}: expected ${argName} to be in this list [${acceptedTypes.join(', ')}] of types but found ${argValueType}`,
          );
        }
      }
    });
